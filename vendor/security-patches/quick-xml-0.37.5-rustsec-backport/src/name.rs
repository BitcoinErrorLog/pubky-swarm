...
pub enum NamespaceError {
    ...
    /// A start tag declared more namespace bindings than the configured limit.
    ///
    /// Contains the configured per-element limit.
    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::TooManyDeclarations(limit) => {
                write!(f, "start tag declares more than {limit} namespace bindings")
            }
        }
    }
}
...
pub(crate) struct NamespaceResolver {
    ...
    nesting_level: i32,
    /// Maximum namespace declarations accepted on one start tag.
    max_declarations_per_element: usize,
}

/// Default per-element namespace declaration limit.
pub(crate) const DEFAULT_MAX_DECLARATIONS_PER_ELEMENT: usize = 256;
...
impl NamespaceResolver {
    ...
    pub fn push(&mut self, start: &BytesStart) -> Result<(), NamespaceError> {
        ...
        for a in start.attributes().with_checks(false) {
            if let Ok(Attribute { key: k, value: v }) = a {
                if k.as_namespace_binding().is_some() {
    ...
    /// Sets the maximum namespace declarations accepted on one start tag.
    #[cfg(test)]
    fn set_max_declarations_per_element(&mut self, limit: usize) -> &mut Self {
...
mod namespaces {
    ...
    fn push_rejects_too_many_declarations() {
        ...
        for index in 0..=DEFAULT_MAX_DECLARATIONS_PER_ELEMENT {
            tag.push_str(&format!(" xmlns:p{index}=''"));
        }
