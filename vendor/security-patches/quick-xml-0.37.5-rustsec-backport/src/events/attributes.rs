...
/// switches from a direct linear scan to a hash pre-filter.
const SMALL_ATTRIBUTE_COUNT: usize = 32;

/// A no-op hasher for values that are already 64-bit hashes.
#[derive(Default)]
...
pub(crate) struct IterState {
    ...
    /// Hashes of `keys`, allocated only after the small-tag threshold.
...
impl IterState {
    ...
    /// Hash-prefiltered duplicate check for start tags with many attributes.
    #[cold]
    fn check_for_duplicates_hashed(
        ...
        let key_hashes = self.key_hashes.get_or_insert_with(|| {
            ...
            for range in keys {
                set.insert(hash_name(&slice[range.clone()]));
            }
            set
        });
        ...
        if !key_hashes.insert(hash_name(&slice[key.clone()])) {
            if let Some(prev) = self
                .keys
                .iter()
                .find(|range| slice[(*range).clone()] == slice[key.clone()])
            {
                return Err(AttrError::Duplicated(key.start, prev.start));
            }
        }
...
mod xml {
    ...
    mod duplicated {
        ...
        mod with_check {
            ...
            /// A duplicate crossing the hash threshold retains its exact
            /// original and duplicate positions.
            #[test]
            fn duplicate_past_hash_threshold() {
                let duplicate = SMALL_ATTRIBUTE_COUNT / 2;
                let count = SMALL_ATTRIBUTE_COUNT + 8;
                ...
                for index in 0..count {
                    ...
                    source.push_str(&format!("k{index:04}=''"));
                }
                source.push(' ');
                let duplicate_position = source.len();
                source.push_str(&format!("k{duplicate:04}=''"));
